    ...
    public function checkUserCredentials($credentials)
    {
        try{
          ...
          if (Auth::attempt($credentials))
            {
                $check_admin = auth()->user()->role != config('settings.roles.names.adminRole') ? auth()->logout() : true;
                throw_if(!$check_admin, new Exception('You are not authorized to access dashboard'));
                return auth()->user();
            }
            return false;
